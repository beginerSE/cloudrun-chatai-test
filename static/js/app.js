...
function createDataTable(data) {
    ...
    html += `<div class="d-flex justify-content-between align-items-center mb-2">`;
    html += `<p class="text-muted mb-0"><i class="bi bi-table"></i> クエリ結果 (${data.length}行)</p>`;
    html += `<button class="btn btn-sm btn-outline-primary download-btn" onclick="downloadCSV(tableDataStore['${tableId}'], 'query_result.csv')">`;
    ...
    if (data.length > maxRows) {
        html += `<p class="text-muted">最初の${maxRows}行のみ表示</p>`;
    }
        ...
        if (data.success && data.messages) {
            ...
            data.messages.forEach(msg => {
                ...
                if (msg.ai_response) {
                    ...
                    if (msgContainer) {
                        let html = `<div class="answer-text mb-3">${formatAnswer(msg.ai_response)}</div>`;
                        
                        // Render data table and charts if query result exists
                        if (msg.query_result) {
                            try {
                                const result = JSON.parse(msg.query_result);
